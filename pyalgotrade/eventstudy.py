...
def build_plot(studyResults):
    # Calculate each value.
    x = []
    y = []
    std = []
    for t in range(studyResults.getLookBack()*-1, studyResults.getLookForward()+1):
        x.append(t)
        values = np.asarray(studyResults.getValues(t))
...
def plot(studyResults):
    """Plots the result of the analysis.

    :param studyResults: The result of the analysis
    :type studyResults: :class:`Results`.
    """
